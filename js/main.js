...
var div = d3.select("#leftBottom").append("div").attr("class", "tooltip").style("opacity", 0);
d3.json("data/data.json", function(error, data) {
	...
	data.forEach(function(d, i) {
		...
		for (var j = 0; j < d.practitioners; j++) {
			d3.select("#leftBottom").append("span").attr("class", "control glyphicon glyphicon-user").attr("style", "color:" + practColor).attr("id", "practitionersOf" + d.sector).on("mouseover", function() {
				div.transition().duration(200).style("opacity", .9);
				div.html(d.sector + "<br/>" + d.practitioners).style("left", (d3.event.pageX) + "px").style("top", (d3.event.pageY - 28) + "px");
...
function mainGraph(data) {
	...
	var mySvg = d3.selectAll("#leftTop").append("svg").attr("id", "spending").attr("width", width).attr("height", height).style("opacity", 0);
	var svg = mySvg.append("g").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
	...
	g.append("path").attr("d", arc).style("fill", function(d) {
		...
		setTimeout(function() {
			...
			d3.select("#leftTop").append("input").attr("type", "button").attr("class", "button").attr("id", "reset").attr("value", "reset").on("click", function() {
				d3.select("#right").selectAll("svg").transition().duration(700).style("opacity", 0);
				d3.select("#backgroundinfo").transition().duration(700).style("opacity", 0);
				d3.selectAll(".button").transition().duration(700).style("opacity", 0);
				setTimeout(function() {
		...
		setTimeout(function() {
			d3.select("#right").selectAll("svg").transition().duration(500).style("opacity", 1);
			d3.select("#right").selectAll(".button").transition().duration(500).style("opacity", 1);
		}, 500);
	});
	g.append("text").attr("transform", function(d) {
		return "translate(" + labelArc.centroid(d) + ")";
	}).attr("dy", ".35em").text(function(d) {
		return d.data.spending;
	});
	var legendG = mySvg.selectAll(".legend") // note appending it to mySvg and not svg to make positioning easier
		.data(pie(data)).enter().append("g").attr("transform", function(d, i) {
	...
	d3.select("#leftTop").selectAll("svg").transition().duration(500).style("opacity", 1);
}
...
function makeBar(config) {
	var margin = {
		...
		svg = d3.select("#data").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).style("opacity", 0),
	...
	d3.csv(config.csv, function(d, i, columns) {
		...
		g.append("g").attr("class", "axis").call(d3.axisLeft(y).ticks(null, "s")).append("text").attr("x", 2).attr("y", y(y.ticks().pop()) + 0.5).attr("dy", "0.32em").attr("fill", "#000").attr("font-weight", "bold").attr("text-anchor", "start").text(config.measure);
...
function makeLine(config) {
	...
	d3.csv(config.csv, type, function(error, data) {
		...
		g.append("g").attr("class", "axis axis--y").call(d3.axisLeft(y)).append("text").attr("transform", "rotate(-90)").attr("y", 6).attr("dy", "0.71em").attr("fill", "#000").text(config.measure);
		var valueLine = g.selectAll(".valueLine").data(lines).enter().append("g").attr("class", "valueLine");
		valueLine.append("path").attr("class", "line").attr("d", function(d) {
			return line(d.values);
		}).attr("id", function(d) {
			return d.id;
		}).attr("active", "off").style("stroke", function(d) {
			return z(d.id);
		});
		...
		function addButton(name) {
			d3.select("#buttons").append("input").attr("type", "button").attr("class", "button").attr("id", name + "button").attr("value", name).attr("style", function() {
